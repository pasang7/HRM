...
class SalarySheetController extends Controller
{
    public function salarySheet(Request $request){
        ...
        $new['incomes'] = PayrollIncome::where('status', 'active')->get();
        ...
        if($is_salary_paid){ //Real Salary
            ...
            $users=User::with('current_salary', 'allowances')
                ...
                if($allow){
                    if($user->current_salary){
                        ...
                        foreach($period as $key=>$date){                            
                            $attendance=Attendance::with('leave')->where('user_id',$user->id)
                                                    ->whereDate('date',$date)
                                                    ->first();
                            if($attendance){
                        ...
                        if($is_joined_this_month){     
                            ...
                            $day_in_month= $companySetting->day_in_month;                                              
                        ...
                        $allowances = PayrollIncome::all();
                        $userAllowance = [];
                        $userAllowances = $user->allowances->pluck('id')->toArray();
                        $grossSalary = $monthly_salary;
                        foreach ($allowances as $allowance) {
                            if (in_array($allowance->id , $userAllowances)) {
                                if ($allowance->calculation_method == 'percent') {
                                    $amount = ($allowance->percent_rate * $monthly_salary) / 100;
                                } else if ($allowance->calculation_method == 'amount'){
