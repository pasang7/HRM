<form action="{{ route('leave.partial.accept') }}" method="POST">
    ...
    <div class="row leavePartial">
        <div class="col-6">
            ...
            <span>Shift: {{ date('h:i A', strtotime($leave->shift->clockin)) }} to
                {{ date('h:i A', strtotime($leave->shift->clockout)) }}</span>
        </div>
        <div class="col-6">
    ...
    <table class="table">
        ...
        <tbody>
            ...
            <td class="text-center">
                <input type="radio" name="x[{{ $begin->format('Y/m/d') }}]" value="1" class="radio-custom radio-green"
                    title="Paid" checked>&nbsp; Paid &nbsp;
                <input type="radio" name="x[{{ $begin->format('Y/m/d') }}]" value="2" class="radio-custom radio-black"
                    title="Unpaid">&nbsp; Unpaid &nbsp;
                <input type="radio" name="x[{{ $begin->format('Y/m/d') }}]" value="3" class="radio-custom radio-red"
                    title="Reject">&nbsp; Reject &nbsp;
            </td>
