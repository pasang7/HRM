$(document).on('click', '.btn-clockin', function () {
    ...
    $.ajax({
        ...
        .done(function (data) {
            ...
            if (res.status) {
                $('#guest-modal-clockin').html(res.view)
                Webcam.set({
                    width: 320,
                    height: 240,
                    image_format: "jpg",
                ...
                $('#guest-modal-clockin').modal('show');
            } else {
...
$(document).on('submit', '#form-clockin', function (e) {
    ...
    $.ajax({
        ...
        .done(function (data) {
            var res = JSON.parse(data)
            if (res.status) {
                showAlert('success', 'Done', 'Clockin successfully')
                location.reload()
            } else {
                showAlert('error', res.title, res.text)
...
$(document).on('submit', '#form-clockout', function (e) {
    ...
    $.ajax({
        ...
        .done(function (data) {
            var res = JSON.parse(data)
            if (res.status) {
                showAlert('success', 'Done', 'Clockout successfully')
                location.reload()
            } else {
                showAlert('error', res.title, res.text)
